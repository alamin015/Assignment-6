        ...
        <div class="card h-100">
                        ...
                        <div class="card-body">
                            <div>
                                <h4>Features</h4>
                                    ${
                                        myFeature ? `<ul class="list-unstyled">
                    
                                        ${`
                                        ${myFeature[0] ? `<li>1. ${myFeature[0]}</li>`:''}
                                        `}
                    
                                        ${`
                                        ${myFeature[1] ? `<li>2. ${myFeature[1]}</li>`:''}
                                        `}
                    
                                        ${`
                                        ${myFeature[2] ? `<li>3. ${myFeature[2]}</li>`:''}
                                        `}
                                        ${`
                                        ${myFeature[3] ? `<li>4. ${myFeature[3]}</li>`:''}
                                        `}
                            ...
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <div>
                                    <span style="width:50px;height:50px;line-height:50px;background:#FEF7F7;cursor:pointer;" class="rounded-circle bg-danger-emphasis d-inline-block text-center text-danger" data-bs-toggle="modal" data-bs-target="#detailsModal" onclick="showDetails('${item.id}')"><i class="fa fa-arrow-right" aria-hidden="true"></i></span>
                                </div>
                            </div>
                        </div>
                    </div>
...
const modalData = (data) => {
    ...
    <div class="p-3 border border-danger-subtle rounded" style="background:#FEF7F7;">
        ...
        <div class="myRow my-3">
            <div class="myCol">
               <p class="mb-0 text-success"> <span>${data.pricing ? data.pricing[0].price : "Free of cost/"} </span> <br> <span>${data.pricing ? data.pricing[0].plan: "Basic"}</span> </p>
            </div>
            <div class="myCol">
            <p class="mb-0 text-warning"> <span>${data.pricing ? data.pricing[1].price : "Free of cost/"} </span> <br> <span>${data.pricing ? data.pricing[1].plan: "Pro"}</span> </p>
            </div>
            <div class="myCol">
            <p class="mb-0 text-danger"> <span>${data.pricing ? data.pricing[2].price : "Free of cost/"} </span> <br> <span>${data.pricing ? data.pricing[2].plan: "Enterprise"}</span> </p>
            </div>
        </div>
        <div class="row">
            <div class="col-6" id="featureId">
                ...
                ${
                    ...
                    ${`
                    ${myFeature[1] ? `<li>${myFeature[1].feature_name}</li>`:''}
                    `}

                    ${`
                    ${myFeature[2] ? `<li>${myFeature[2].feature_name}</li>`:''}
                    `}

                    ${`
                    ${myFeature[3] ? `<li>${myFeature[3].feature_name}</li>`:''}
                    `}
                    ${`
                    ${myFeature[4] ? `<li>${myFeature[4].feature_name}</li>`:''}
                    `}
...
<div class="col-12 col-md-5">
    <div class="p-3 border border-secondary-subtle rounded">
        ...
        <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Ipsa, odio.</p>
    </div>
</div>
